...
- Вместо `<amount>` в любой команде можно использовать математические выражения, а так же суффиксы величин (`k`, `m` и т.д.), например `2k + 30*5`
...
Показывает бонус, основанный на уже заполненной квоте и предметах на кассе; деньги в терминале + предметы на кассе; сумму и того, и того; и, если `<amount>` был указан, сколько денег нужно оставить в терминале, чтобы получить `<amount>` после вылета
